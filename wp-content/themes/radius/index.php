			...
			<div class="container">					
				<div class="content">
					<div class="page-title">
						<?php if(is_search()) { ?>
							<h1><?php /* Search Count */ $allsearch = &new WP_Query("s=$s&showposts=-1"); $count = $allsearch->post_count; echo $count . ' '; wp_reset_query(); ?><?php _e('Search results for','slate'); ?> "<?php the_search_query() ?>" </h1>
						<?php } else if(is_tag()) { ?>
							<h1><?php _e('Tag','slate'); ?>: <?php single_tag_title(); ?></h1>
						<?php } else if(is_day()) { ?>
							<h1> <?php _e('Archive','slate'); ?>: <?php echo get_the_date(); ?></h1>
						<?php } else if(is_month()) { ?>
							<h1><?php _e('Archive','slate'); ?>: <?php echo get_the_date('F Y'); ?></h1>
						<?php } else if(is_year()) { ?>
							<h1><?php _e('Archive','slate'); ?>: <?php echo get_the_date('Y'); ?></h1>
						<?php } else if(is_404()) { ?>
							<h1><?php _e('404 - Page Note Found','slate'); ?></h1>
						<?php } else if(is_category()) { ?>
							<h1><?php _e('Category','slate'); ?>: <?php single_cat_title(); ?></h1>
						<?php } else if(is_author()) { ?>
							<h1><?php _e('Posts by Author','slate'); ?>: <?php the_author_posts(); ?> <?php _e('posts by','slate'); ?> <?php
					...
					<!-- 404 page code -->
					<?php if(is_404()) { ?>
						<div class="intro"><?php _e('Sorry, but the page you are looking for is no longer here. Please use the navigations or the search to find what what you are looking for.','slate'); ?></div>
